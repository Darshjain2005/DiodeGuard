    ...
    return (
        ...
        >
            ...
            <div className="text-right">
                <span className="font-mono block" style={{ fontSize: 12, color: '#475569' }}>{log.time}</span>
                {log.date && (
                    <span className="font-mono block" style={{ fontSize: 10, color: '#334155' }}>{log.date}</span>
                )}
            </div>
        </motion.div>
    );
...
const SpamShield = () => {
        ...
        try {
            ...
            if (data.success && data.data) {
                const mappedLogs = data.data.map(log => {
                    // Use the real email sent date (received_at), fall back to DB insert time
                    const realDate = new Date(log.received_at || log.created_at);
                    return {
                        id: log.id || Math.random().toString(),
                        sender: log.sender,
                        subject: log.subject,
                        action: log.classification === 'Spam' ? 'Quarantined' : 'Allowed',
                        risk: log.classification === 'Spam' ? 'High' : 'Low',
                        time: realDate.toLocaleTimeString('en-US', {
                            hour: '2-digit', minute: '2-digit', second: '2-digit',
                            hour12: true
                        }),
                        date: realDate.toLocaleDateString('en-US', {
    ...
    const counts = {
        total: logs.length,
        blocked: logs.filter(l => l.action === 'Quarantined').length,
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <div>
                            <p className="font-mono font-bold" style={{ fontSize: 13, color: '#f1f5f9' }}>User</p>
                            <p className="font-mono" style={{ fontSize: 11, color: '#475569' }}>Honeypot</p>
                        </div>
                    </div>
                </motion.div>
